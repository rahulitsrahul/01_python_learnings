...
class player(object):
    ...
    def draw(self, win):
        ...
        # Draw a bounding rectangle around the character
        self.hitbox = (self.x+20, self.y+10, self.width-40, self.height-10)
        pygame.draw.rect(win,color=(255,0,0), rect=self.hitbox, width=2)
...
class projectice(object):
    def __init__(self, x=200, y=400, radius=10, color=(0,0,0), facing=1):
        ...
        self.vel = 8
        self.hit = False # bullet hit status
            
    def draw(self, win):
        pygame.draw.circle(win, self.color, (self.x, self.y), self.radius)

class pedestral(object):
    def __init__(self, x, y, width, height):
        self.x = x
        self.y = y
        self.width = width
        self.height = height 
        self.hit = False
        self.colors_list = [(0,0,255), (80,255,0), (120, 180, 23), (150, 10, 93)]
        self.colors = itertools.cycle(self.colors_list)
        self.color = next(self.colors)
    def draw(self, win):
        pygame.draw.rect(win,color=self.color, rect=(self.x, self.y, self.width, self.height), width=0)
        if self.hit:
            self.on_hit()
...
def update_pedestral(ped):
    pass

def operate_bullets(keys, man, ped):
    global bullets, trigger
        
    def is_touched_border(bullet):
        if ((bullet.x < 0) | (bullet.x > 500) | bullet.hit==True):
            return True
        else:
            return False
    ...
    if keys[pygame.K_SPACE] & (trigger == False):
        ...
        trigger = True
        bullets[0].x = man.x
        bullets[0].y = man.y + 30 # offsetting 30 units down
    ...
    if trigger:
        if (not is_touched_border(bullets[0])):
            ...
            ped_limits = [ped.x, ped.x+ped.width]
            ped_x1 = ped.x
            ped_x2 = ped.x+ped.width
            ped_y = ped.y
            if ((bullets[0].x > ped_x1) & (bullets[0].x < ped_x2) & (bullets[0].y >ped_y)):
                print("bullet hit pedestral")
                bullets[0].hit = True
            
        else:
            if bullets[0].hit:
                ped.color = next(ped.colors)
...
if __name__ == "__main__":
    ...
    # Initalize pedestral
    ped_block = pedestral(x=50,y=(410-100), width=64, height=(64+100))
    ...
    while run:
        ...
        # Operate Bullets
        operate_bullets(keys=keys, man=man, ped=ped_block)
        # Update Pedestral
        update_pedestral(ped=ped_block)
        # Update Game Window
        redrawGameWindow()
